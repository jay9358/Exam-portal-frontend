...
export default function CityManagerReport() {
    ...
    // Fetch city manager details and their city
    const fetchCityManagerDetails = async () => {
        try {
            ...
            const response = await axios.get(
                `${import.meta.env.VITE_API_URL}/v1/admin/users/${userId}`,
                {
                    headers: {
            ...
            setCityManager(response.data.user);
            console.log("response.data.user", response.data.user);
        } catch (error) {
            console.error("Error fetching city manager details:", error);
            toast.error("Error fetching city manager details.");
        }
    };
    ...
    const fetchSchools = async () => {
        try {
            const response = await axios.get(
                `${import.meta.env.VITE_API_URL}/v1/citymanager/schools`,
                {
                    params: {
                        State: cityManager?.State,
    ...
    const handleFetchExams = async () => {
        try {
            const response = await axios.get(
                `${import.meta.env.VITE_API_URL}/v1/citymanager/exams`,
                {
                    headers: {
                        Authorization: localStorage.getItem("token"),
                    },
    ...
    const handleGenerateReport = async (e) => {
        ...
        try {
            const response = await axios.get(
                `${import.meta.env.VITE_API_URL}/v1/citymanager/reports/performance`,
                {
                    params: {
                        state: cityManager?.State,
                        city: cityManager?.City,
    ...
    return (
        <div className="performance-reports">
            <div className="report-header">
                <h2>City Performance Reports</h2>
                <p>Generate performance reports for {cityManager?.City}</p>
            </div>
